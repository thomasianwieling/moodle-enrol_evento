...
class mod_evento_advanced_testcase extends advanced_testcase {
    ...
    protected function setUp() {
        ...
        $evento_personen_anmeldung = $builder->add_personen_anmeldung("2019-02-17T00:00:00.000+01:00", "hoferlis", "2018-06-05T08:58:20.723+02:00", "auto" , 415864, 20216, 25491, 118000, $evento_status);
        $evento_person = $builder->add_person("Max", "Fritz", "max.fritz@htwchur.ch",  118000, 30040, true, 118000, $evento_personen_anmeldung);
        $ad_account = $builder->add_ad_account(0, "2019-02-17T00:00:00.000+01:00", "2019-02-17T00:00:00.000+01:00", 0, 118000, 0, 1, 0, "S-1-5-21-2460181394-1097805571-3701207438-51315", "MisTe");
    ...
    public function get_user() {
        ...
        /*Get new user*/
        $eventoperson = $this->locallib->get_user_exposed($personid1, $isstudent = true, $username = null);
        $eventoperson = $this->locallib->get_user_exposed(117828, $isstudent = true, $username = null);
        /*Get Database Records*/

        $search = "S-1-5-21-";
        $replace = null;
        $username1 = str_replace($search, $replace, $this->get_username_from_person_id($personid1).$this->simulator->config->adshibbolethsuffix);
        $username2 = str_replace($search, $replace, $this->get_username_from_person_id($personid2).$this->simulator->config->adshibbolethsuffix);
        ...
        $this->assertEquals($user1->email, $this->get_mail_from_person_id($personid1));
        $this->assertEquals($user2->email, $this->get_mail_from_person_id($personid2));
    }
